...
public class WorkerHandler {
    ...
    private byte[] serializeHistogram(Histogram inputHisto) {
        ...
        while (true) {
            final int outBytes = inputHisto.encodeIntoCompressedByteBuffer(histogramSerializationBuffer);
            Preconditions.checkState(outBytes == histogramSerializationBuffer.position());
            final int capacity = histogramSerializationBuffer.capacity();
            ...
            histogramSerializationBuffer = ByteBuffer.allocate(capacity * 2);
        }
        byte encodedBuffer[] = new byte[histogramSerializationBuffer.position()];
        histogramSerializationBuffer.flip();
        histogramSerializationBuffer.get(encodedBuffer);
